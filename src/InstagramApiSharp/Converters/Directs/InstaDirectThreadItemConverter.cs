...
namespace InstagramApiSharp.Converters
{
    internal class InstaDirectThreadItemConverter : IObjectConverter<InstaDirectInboxItem, InstaDirectInboxItemResponse>
    {
        ...
        public InstaDirectInboxItem Convert()
        {
            ...
            else if (threadItem.ItemType == InstaDirectThreadItemType.VideoCallEvent && SourceObject.VideoCallEvent != null)
            {
                try
                {
                    threadItem.VideoCallEvent = ConvertersFabric.Instance.GetVideoCallEventConverter(SourceObject.VideoCallEvent).Convert();
                }
                catch { }
            }
            else if (threadItem.ItemType == InstaDirectThreadItemType.ProductShare && SourceObject.ProductShare != null)
            {
                try
                {
                    threadItem.ProductShare = ConvertersFabric.Instance.GetProductConverter(SourceObject.ProductShare.Product).Convert();
                }
                catch { }
            }
            else if (threadItem.ItemType == InstaDirectThreadItemType.ArEffect && SourceObject.ArEffect != null)
            {
                try
                {
                    threadItem.ArEffect = ConvertersFabric.Instance.GetDirectArEffectConverter(SourceObject.ArEffect).Convert();
                }
                catch { }
            }
            return threadItem;
