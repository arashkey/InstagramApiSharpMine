...
namespace InstagramApiSharp.API.Processors
{
    ...
    internal class MessagingProcessor : IMessagingProcessor
    {
        ...
        /// <param name="module">Module => Default is ClipsViewerClipsTab</param>
        public async Task<IResult<InstaDirectRespondPayload>> SendDirectReelClipsToRecipientsAsync(string text,
            string mediaPk,
            string[] recipients,
            InstaMediaContainerModuleType module = InstaMediaContainerModuleType.ClipsViewerClipsTab) =>
            await SendDirectReelClips(text, mediaPk, null, recipients, module).ConfigureAwait(false);
        ...
        public async Task<IResult<InstaSharing>> ShareUserAsync(string userIdToSend, string threadId)
        {
            ...
            try
            {
                ...
                if (!_httpHelper.NewerThan180)
                {
                    requestContent.Add(new StringContent(_user.CsrfToken), "\"_csrftoken\"");
                }
        ...
        private async Task<IResult<bool>> EnableDisableVanishMode(string threadId, bool flag)
        {
            UserAuthValidator.Validate(_userAuthValidate);
            try
            {
